...
Alterado em 2016 para tratar valores numéricos não monentário por Diogo Alves 
*/
...
public class ExtensoWriter {
	...
	private static final String[] FRACAO_SINGULAR = {"centavo","centésimo"};
	private static final String[] FRACAO_PLURAL = {"centavos", "centésimos"};
	
	private static final String PARTICULA_ADITIVA = "e";
	private static final String PARTICULA_DESCRITIVA = "de";
	
	public static final int CONVERSAO_MOEDA = 0;
	public static final int CONVERSAO_NUMERICA = 1;
	...
	private static ExtensoWriter instance = null;
	
	public static ExtensoWriter getInstance() {
		if (instance == null) {
			instance = new ExtensoWriter();
		}

		return instance;
	}
	
	private ExtensoWriter() {
		preencherGrandezasPlural();
		preencherGrandezasSingular();
		preencherNomes();
	}

	public String write(final BigDecimal amount, int tipoConversao) {
		...
		while (!decomposed.empty()) {
			...
			case 0:
				...
				if (BigInteger.ONE.equals(parteInteira)) {
					sb.append(" ").append(MOEDA_SINGULAR[tipoConversao]);
				} else if (parteInteira.compareTo(BigInteger.ZERO) > 0) {
					if (lastNonZeroExponent >= 6) {
						//sb.append(" ").append(PARTICULA_DESCRITIVA);
					}
					sb.append(" ").append(MOEDA_PLURAL[tipoConversao]);
				}
				break;
				
			case -3:
				if (1 == valor) {
					sb.append(" ").append(FRACAO_SINGULAR[tipoConversao]);
				} else if (valor > 1) {
					sb.append(" ").append(FRACAO_PLURAL[tipoConversao]);
				}
				break;
			}
